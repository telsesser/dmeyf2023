...
EstimarGanancia_lightgbm <- function(x) {
  ...
  tbl <- copy(dataset_test[, list("gan" = ifelse(clase_ternaria == "BAJA+3",
    PARAM$hyperparametertuning$POS_ganancia,
    PARAM$hyperparametertuning$NEG_ganancia
  ))])
...
dataset[, clase01 := ifelse(clase_ternaria %in% c("BAJA+3"), 1L, 0L)]
...
dataset[
  foto_mes %in% PARAM$input$training &
    (azar <= PARAM$trainingstrategy$undersampling | clase_ternaria %in% c("BAJA+1", "BAJA+2", "BAJA+3")),
  training := 1L
]
